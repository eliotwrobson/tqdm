...
def doc2rst(doc, arglist=True, raw=False):
    ...
    if raw:
        ...
        if arglist:
            doc = "\n".join(
                i if not i or i[0] == " " else "* " + i + "  " for i in doc.split("\n")
            )
    return doc
...
DOC_tqdm_init_args = (
    DOC_tqdm_init.partition(doc2rst(HEAD_ARGS))[-1]
    .replace("\n      ", "\n    ")
    .replace("\n      ", "\n    ")
)
DOC_tqdm_init_args, _, DOC_tqdm_init_rets = DOC_tqdm_init_args.partition(
    doc2rst(HEAD_RETS)
)
...
if __name__ == "__main__":
    (src_dir.parent / "README.md").write_text(README_rst, encoding="utf-8")
